...
public abstract class AndroidJApplet extends Activity {
    ...
    public static class Graphics extends View{
		...
		/**
		 * draws the outline of a triangle from the left vertex.
		 *         *
		 *       *   *
		 * @param x
		 * @param y
		 ...
		 * @param height
		 */
		public void drawIsoTriangle(int x, int y, int width, int height){
		    float[] pts ={x,y, x + width, y,//base line
		                x+width,y, (float) (x+(width/2.0)), y-height, //right line
		               (float)( x+(width/2.0)),y -height, x,y//left line
		    };
		    mCanvas.drawLines(pts, 0, 12, mOutline);
		}
